        ...
        match id {
            ...
            0x2 => {
                match val {
                    ...
                    _ => {
                        return Err(Error::new(ErrorKind::Protocol,
                                              "Invalid Value for enable push setting in settings \
                                               frame"))
                    }
                }
            }
            ...
            0x4 => {
                if val > MAX_FLOW_CONTROL_WINDOW_SIZE {
                    return Err(Error::new(ErrorKind::FlowControl,
                                          "Initial window size must be lower than 2^31-1 octets \
                                           in settings frame"));
                }
            ...
            0x5 => {
                if val < MIN_FRAME_SIZE || val > MAX_FRAME_SIZE {
                    return Err(Error::new(ErrorKind::Protocol,
                                          "Max frame size must be between 2^14 and 2^24-1 \
                                           octets in settings frame"));
                }
